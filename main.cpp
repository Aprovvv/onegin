...
static void read_text_from_file(char oneg[STR_COUNT][STR_LEN], FILE* pointer_to_onegin);
static void sort_oneg(char oneg[STR_COUNT][STR_LEN]);
void swap(void* a, void* b, size_t size);
static void print_oneg(char oneg[STR_COUNT][STR_LEN]);
static void parse_buffer(char* str, size_t size);
...
int main()
{
    ...
    stat("parsed_oneg_1.txt", &onegin_data);
    char* buf_p = (char*)calloc(onegin_data.st_size + 1, 1);
    read_text_from_file(oneg, pointer_to_onegin);
    sort_oneg(oneg);
    print_oneg(oneg);
    fclose(pointer_to_onegin);
}

static void read_text_from_file(char** p, char* name)
{
...
static void sort_oneg(char oneg[STR_COUNT][STR_LEN])
{
    for (int i = 0; i < STR_COUNT - 1; i++)
    {
        for (int j = 0; j < STR_COUNT - 1; j++)
        {
            ...
            parse_buffer(s1, STR_LEN);
            parse_buffer(s2, STR_LEN);
            if (strcmp(s1, s2) > 0)
